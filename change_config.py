...
def __cli():
    ...
    parser.add_argument(
        "--DB_CACHE", type=lambda x: (str(x).lower() == "true"), default=False
    )
    parser.add_argument(
        "--DB_REPLACE", type=lambda x: (str(x).lower() == "true"), default=False
    )
    parser.add_argument("--DB_time", default=1, type=float)
    parser.add_argument(
        "--biosensor", type=lambda x: (str(x).lower() == "true"), default=False
    )
    parser.add_argument(
        "--use_cache", type=lambda x: (str(x).lower() == "true"), default=False
    )
    parser.add_argument(
        "--add_Hs", type=lambda x: (str(x).lower() == "true"), default=False
    )
    parser.add_argument(
        "--use_transpositions", type=lambda x: (str(x).lower() == "true"), default=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--folder_to_save", default=os.path.dirname(os.path.abspath(__file__))
    )
    args = parser.parse_args()

    def change_dB_setting(
        DB_CACHE,
        DB_REPLACE,
        DB_time,
        biosensor,
        ...
        with open(
            "{}/data/base_config.py".format(os.path.dirname(os.path.abspath(__file__))),
            "r",
        ) as file_original:
        ...
        with open("{}/config.py".format(folder_to_save), "w") as replacement_text:
            ...
            if DB_CACHE:
                if "DB_CACHE = True" not in whole_text:
                    whole_text = whole_text.replace(
                        "DB_CACHE = False", "DB_CACHE = True"
                    )
            else:
                if "DB_CACHE = False" not in whole_text:
                    whole_text = whole_text.replace(
                        "DB_CACHE = True", "DB_CACHE = False"
                    )
            # Changing DB replace
            if DB_REPLACE:
                if "DB_REPLACE = True" not in whole_text:
                    whole_text = whole_text.replace(
                        "DB_REPLACE = False", "DB_REPLACE = True"
                    )
            else:
                if "DB_REPLACE = False" not in whole_text:
                    whole_text = whole_text.replace(
                        "DB_REPLACE = True", "DB_REPLACE = False"
                    )
            # Changing DB_time:
            whole_text = re.sub(
                "DB_time = \d+.\d+", "DB_time = {}".format(DB_time), whole_text
            )
            ...
            if biosensor:
                if "biosensor = True" not in whole_text:
                    whole_text = whole_text.replace(
                        "biosensor = False", "biosensor = True"
                    )
                    whole_text = whole_text.replace(
                        "retrosynthesis = True", "retrosynthesis = False"
                    )
            else:
                if "biosensor = False" not in whole_text:
                    whole_text = whole_text.replace(
                        "biosensor = True", "biosensor = False"
                    )
                    whole_text = whole_text.replace(
                        "retrosynthesis = False", "retrosynthesis = True"
                    )
            # Changing use_cache
            if use_cache:
            ...
            if use_transpositions:
                if "use_transpositions = True" not in whole_text:
                    whole_text = whole_text.replace(
                        "use_transpositions = False", "use_transpositions = True"
                    )
            else:
                if "use_transpositions = False" not in whole_text:
                    whole_text = whole_text.replace(
                        "use_transpositions = True", "use_transpositions = False"
                    )
